/**
 *
 * Returns an array containing cells in a topological order -- furthest back cells first.
 * If cells are added to the sheet in this order, no issues should occur.
 *  @param {Array} cells Array of cells to order
 *  @returns Array of arrays in the format `[String cellType, Object cell]` in a safe order to add
 */
 export function getSafeCellAddOrder(cells) {
    ...
    while (cells.length > 0) {
        ...
        //skip the cell for now if its parent hasn't been added
        if (cell.parent && !ids.hasOwnProperty(cell.parent)) {
            cells.push(cell);
            continue;
        }

        //if we get here, the cell must be on the furthest back level that we haven't processed yet
        // (all its parents have been processed), so it is safe to add this one
        if (type === "dia.Element.Cut") {
            order.push(['cut', cell]);
            ids[cell.id] = true;
        }
        else if (type === "dia.Element.Atomic") {
            order.push(['atomic', cell]);
            ids[cell.id] = true;
        }
    }
    return order;
